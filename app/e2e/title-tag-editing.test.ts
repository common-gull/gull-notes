...
test.describe('Title and Tag Editing', () => {
	...
	test('should create vault, note, and edit title', async ({ page }) => {
		...
		await expect(page.getByRole('heading', { name: 'Notes', exact: true })).toBeVisible({ timeout: 10000 });
		
		// Wait for UI to settle
		await page.waitForTimeout(1000);
		
		// Create a new note - find button with Plus icon
		const createNoteButton = page.locator('aside').getByRole('button').filter({ hasText: '' }).first();
		...
		await expect(noteHeader.locator('button').filter({ hasText: 'My First Note' })).toBeVisible({ timeout: 3000 });
	...
	test('should add and manage tags', async ({ page }) => {
		...
		await expect(page.locator('.bg-secondary').filter({ hasText: 'important' })).toBeVisible({ timeout: 2000 });
		
		// Add second tag
		await tagInput.fill('work');
		await tagInput.press('Enter');
		await expect(page.locator('.bg-secondary').filter({ hasText: 'work' })).toBeVisible({ timeout: 2000 });
		
		// Save tags
		await page.getByRole('button', { name: 'Save Tags' }).click();
		
		// Verify dialog closed
		await expect(page.getByRole('heading', { name: 'Manage Tags' })).not.toBeVisible({ timeout: 2000 });
		
		// Check if tags appear in the header
		await expect(page.locator('div.border-b').locator('.bg-secondary').first()).toBeVisible({ timeout: 3000 });
	});
	...
	test('should show tag autocomplete with existing tags', async ({ page }) => {
		...
		await expect(dialog.locator('.bg-secondary').filter({ hasText: 'important' })).toBeVisible({ timeout: 2000 });
	});
	...
	test('should persist title and tags across note switches', async ({ page }) => {
		...
		const headerTitleButton2 = page.locator('div.border-b').locator('button').filter({ hasText: 'Untitled Note' });
		...
		await expect(page.locator('button').filter({ hasText: 'Note One' }).first()).toBeVisible({ timeout: 3000 });
		
		// Verify first note tag is still there
		await expect(page.locator('div.border-b').locator('.bg-secondary').filter({ hasText: 'personal' })).toBeVisible({ timeout: 3000 });
		...
		await expect(page.locator('button').filter({ hasText: 'Note Two' }).first()).toBeVisible({ timeout: 3000 });
	});
});
